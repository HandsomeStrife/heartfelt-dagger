// scripts/index-docs.mjs (Node 18+)
...
const START_URL = process.env.DOCS_START_URL || 'http://localhost:8090/reference/';
...
for (const page of REFERENCE_PAGES) {
    ...
    try {
        ...
        const { data: html } = await axios.get(url, { 
            timeout: 15000,
            headers: {
                'User-Agent': 'DaggerHeart-Indexer/1.0'
            }
        });
...
console.log('✓ Search index ready for deployment');
