    ...
    public function showJoin(Request $request, string $invite_code)
    {
        ...
        if ($user) {
            ...
            if ($anonymousParticipantId) {
                $participant = $room->activeParticipants()
                    ...
                    ->where('user_id', null)
                    ->first();
                $isAlreadyParticipating = !!$participant;
                $participantCheckDetails['participant_found'] = !!$participant;
                if ($participant) {
        ...
        if ($isAlreadyParticipating) {
            ...
            Log::info('Room join page - Already participating, checking password', [
                'room_id' => $room->id,
                'user_id' => $userId,
                'room_has_password' => !!$room->password,
                'password_in_url' => !!$passwordFromUrl,
    ...
    public function session(Request $request, Room $room)
    {
        ...
        if ($user) {
            ...
            if ($anonymousParticipantId) {
                ...
                $participationDetails['participant_found'] = !!$participant;
                if ($participant) {
