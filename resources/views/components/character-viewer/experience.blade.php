        ...
        <ul pest="experience-list" class="mt-2 divide-y divide-slate-800/80">
            @foreach ($character->experiences as $index => $experience)
                <li pest="experience-item-{{ $index }}" class="flex items-center justify-between py-2">
                    <span pest="experience-name">{{ $experience['name'] }}</span>
                    <div class="flex items-center gap-2">
                        ...
                        @endif
                        <span pest="experience-modifier" class="px-2 py-1 text-xs rounded-md bg-slate-800/80 ring-1 ring-slate-700/60">+{{ $character->getExperienceModifier($experience['name'] ?? '') }}</span>
                    </div>
                </li>
            @endforeach
        </ul>
