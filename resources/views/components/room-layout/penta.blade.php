...
<!-- 5 Participants - 2x3 Grid with Reserved Void Slot -->
<div class="h-full w-full grid grid-cols-3 grid-rows-2 gap-1">
    ...
    <!-- Bottom Row - Reserved Void + 2 Video Slots -->
    ...
    @foreach($participants->skip(3)->take(2) as $index => $participant)
        <x-room-video-slot :slot-id="$index + 5" :participant="$participant" :is-host="false" />
    @endforeach
    
    {{-- Fill remaining bottom row slots with empty slots --}}
    @for($i = $participants->skip(3)->count(); $i < 2; $i++)
        <x-empty-room-slot :slot-id="$i + 5" />
    @endfor
</div>
