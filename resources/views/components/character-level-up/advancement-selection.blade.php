@props(['tierOptions', 'stepNumber', 'stepKey', 'character', 'getAllCharacterExperiences'])

@php
    $stepLabel = $stepNumber === 1 ? 'First' : 'Second';
    $advancement = $stepNumber === 1 ? 'firstAdvancement' : 'secondAdvancement';
@endphp

<div class="space-y-6">
    <div>
        <h3 class="font-outfit font-bold text-slate-100 text-xl mb-3">Choose Your {{ $stepLabel }} Advancement</h3>
        <p class="text-slate-400">Select your {{ strtolower($stepLabel) }} advancement from the options below.</p>
    </div>
    ...
    <div class="space-y-4">
        ...
        <div x-data="{
             ...
             x-init="
                $watch('{{ $advancement }}', () => {
                    isSelected = {{ $advancement }} === optionIndex;
                });
                isSelected = {{ $advancement }} === optionIndex;
             "
             ...
             @click="selectAdvancement({{ $index }}, '{{ strtolower($stepLabel) }}')">
            ...
            <!-- Experience Bonus Selection Interface -->
            <x-character-level-up.experience-bonus-selection 
                :index="$index" 
                :option="$option" 
                :selected="$advancement"
                :getAllCharacterExperiences="$getAllCharacterExperiences" />
        </div>
        @endforeach
    </div>
    @endif
