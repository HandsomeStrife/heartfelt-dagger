                        ...
                        <div class="bg-slate-900/80 backdrop-blur-xl border border-slate-700/50 rounded-xl p-8">
                            ...
                            <div class="prose prose-invert max-w-none">
                                <p class="text-slate-300 leading-relaxed mb-8">
                                    Ancestries represent the biological heritage of your character, providing unique features and capabilities.
                                </p>

                                <!-- Ancestries List -->
                                <div class="space-y-8">
                                    @foreach($ancestries ?? [] as $ancestryKey => $ancestryData)
                                        <div class="bg-slate-800/50 border border-slate-600/30 rounded-xl p-6">
                                            <h3 class="font-outfit text-2xl font-bold text-amber-400 mb-4">{{ $ancestryData['name'] }}</h3>
                                            <p class="text-slate-300 leading-relaxed mb-6">{{ $ancestryData['description'] }}</p>
                                            
                                            @if(isset($ancestryData['features']) && is_array($ancestryData['features']))
                                                <div>
                                                    <h4 class="font-outfit text-lg font-semibold text-white mb-4">Ancestry Features</h4>
                                                    <div class="space-y-4">
