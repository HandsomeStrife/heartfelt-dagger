...
<div x-cloak>
    ...
    <div x-show="selected_subclass" class="mb-6 p-4 bg-gradient-to-r from-emerald-500/10 to-green-500/10 border border-emerald-500/20 rounded-xl">
        <div class="flex items-center">
            ...
            <div>
                ...
                <p class="text-slate-300 text-sm">You have chosen <span x-text="selectedSubclassData?.name || ''"></span></p>
            </div>
        </div>
    </div>
    ...
    <div x-show="selected_class" class="space-y-6">
                    ...
                    >
                        ...
                        <div class="p-6">
                            ...
                            <div class="mb-6">
                                ...
                                <p class="text-slate-300 text-base leading-relaxed mb-4" x-text="subclassData.description"></p>
                                
                                <!-- Spellcast Trait -->
                                <template x-if="subclassData.spellcastTrait">
                                    <div class="inline-flex items-center px-3 py-1 bg-purple-500/20 border border-purple-500/30 rounded-lg">
                                        <span class="text-purple-300 text-sm font-medium" x-text="`Spellcast: ${subclassData.spellcastTrait.charAt(0).toUpperCase() + subclassData.spellcastTrait.slice(1)}`"></span>
                                    </div>
                                </template>
                            </div>
                            ...
                            <div class="space-y-6">
                                <!-- Foundation Features -->
                                <template x-if="subclassData.foundationFeatures && subclassData.foundationFeatures.length > 0">
                                    <div>
                                        <h5 class="text-emerald-400 font-semibold text-base font-outfit mb-3">Foundation Features</h5>
                                        <div class="space-y-3">
                                            <template x-for="feature in subclassData.foundationFeatures" :key="feature.name">
                                                <div class="bg-slate-800/30 border border-slate-700/50 rounded-lg p-4">
                                                    <div class="text-white font-semibold text-sm mb-2" x-text="feature.name"></div>
                                                    <div class="text-slate-300 text-sm leading-relaxed prose prose-slate prose-sm max-w-none" x-html="feature.description"></div>
                                                </div>
                                            </template>
                                        </div>
                                    </div>
                                ...
                                <template x-if="subclassData.specializationFeatures && subclassData.specializationFeatures.length > 0">
                                    <div>
                                        <h5 class="text-amber-400 font-semibold text-base font-outfit mb-3">Specialization Features</h5>
                                        <div class="space-y-3">
                                            <template x-for="feature in subclassData.specializationFeatures" :key="feature.name">
        ...
        <template x-if="selected_class && (!availableSubclasses || Object.keys(availableSubclasses).length === 0)">
